...
def write_sector_12(connection, card_info):
    sector = 12
    ...
    uid = card_info.card_uid.encode('ascii')
    secret_code = card_info.get_secret_code().encode('ascii')
    expiration_date = card_info.expiration_date.strftime("%Y%m%d").encode('ascii')

    data_blocks = [uid, secret_code, expiration_date]

    for block_offset, data in enumerate(data_blocks):
        global_block = sector * 4 + block_offset
        block_data = data.ljust(16, b'\x00')  # Assurez-vous que les données font 16 octets
        if not write_block(connection, global_block, block_data):
            print(f"Échec de l'écriture dans le bloc {global_block}.")
            return False

        # Vérification de l'écriture
        read_data = read_block(connection, global_block)
        if read_data != list(block_data):
            print(f"Erreur lors de la vérification du bloc {global_block}.")
            return False

    print(f"Données écrites avec succès dans le secteur {sector}.")
    return True

def write_card_sector_12(card_info):
    try:
        available_readers = readers()
        if not available_readers:
            return False, "Aucun lecteur de carte n'a été trouvé. Veuillez brancher un lecteur."
        ...
        success = write_sector_12(connection, card_info)

        return success, "Données écrites avec succès dans le secteur 12." if success else "Échec de l'écriture des données dans le secteur 12."
...
def read_sector_12():
    try:
        # Trouver les lecteurs disponibles
        available_readers = readers()
        if not available_readers:
            return False, "Aucun lecteur de carte n'a été trouvé. Veuillez brancher un lecteur."
        ...
        # Définir la clé par défaut (vous pouvez la changer si nécessaire)
        key = [0xFF] * 6
        ...
        if not sector_data:
            return False, "Impossible de lire les données du secteur 12."

        # Interpréter les données lues
        card_data = {}
        for block, data in sector_data.items():
            if block == sector * 4:  # Premier bloc du secteur 12
                card_data['uid'] = data['ascii'].strip('\x00')
            elif block == sector * 4 + 1:  # Deuxième bloc du secteur 12
                card_data['secret_code'] = data['ascii'].strip('\x00')
            elif block == sector * 4 + 2:  # Troisième bloc du secteur 12
                card_data['expiration_date'] = data['ascii'].strip('\x00')
